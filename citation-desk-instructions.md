...
Project knowledge. Apply the two files in this project's knowledge. Use the common-knowledge guardrail to avoid over-flagging claims that are genuinely common knowledge in enterprise IT. Use Will's voice standards whenever you rewrite a paraphrase-risk passage, so the rewrite sounds like him.
...
- Common knowledge in enterprise IT that is broadly known, non-controversial, and not traceable to a single origin. Apply the common-knowledge guardrail file for the precise boundary.
